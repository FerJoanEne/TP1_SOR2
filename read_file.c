...
void mostrarDirectorios(char ruta[], unsigned short cant_entries, FILE * in, Fat12BootSector bs, long int offset) {
    ...
    for(i=0; i<cant_entries; i++) {      
        ...
        case ARCHIVO:
            ...
            offset_buffer = inicio_clusters+ ((numero_cluster)-2)*bs.sectors_per_cluster* bs.sector_size;

            // Mover la posición actual del puntero de archivo a la posición de memoria del archivo
            fseek(in, offset_buffer, SEEK_SET);
            
            // Leer el contenido del archivo en un buffer en memoria
            fread(buffer, pentry->file_size, 1, in);

            buffer[pentry->file_size] = '\0';
